...
NEW: feature mapping outputs
- Global feature mapping JSON:
    feature_mapping.json
- Per-k feature mapping JSON in each k-folder:
    feature_mapping_k_<k>.json

The per-k feature mapping JSON includes:
- features used
- scaler parameters
- PCA 2D mapping
- cluster centers in:
    * PCA-2D space
    * standardized/original feature space (inverse projected from PCA2)
- ranked attributes per cluster:
    * strongest positive standardized attributes
    * strongest negative standardized attributes
    * strongest absolute standardized attributes
...
# -----------------------------
def rank_cluster_features_from_center(
    ...
    """
    Rank attributes for one cluster using its center projected back into:
    - standardized feature space
    - original feature space

    Since standardized features are centered around 0, large positive/negative values
    indicate attributes most characteristic of the cluster.
    """
    order_abs = np.argsort(np.abs(center_std))[::-1][:top_n]
    pos_idx = np.argsort(center_std)[::-1]
    pos_idx = pos_idx[center_std[pos_idx] > 0][:top_n]
    neg_idx = np.argsort(center_std)
    neg_idx = neg_idx[center_std[neg_idx] < 0][:top_n]

    def pack(indices):
        return [
            {
                "feature": feature_names[j],
                "center_standardized": float(center_std[j]),
                "center_original_scale": float(center_orig[j]),
                "abs_center_standardized": float(abs(center_std[j])),
            }
            for j in indices
        ]

    return {
        "top_absolute_attributes": pack(order_abs),
        "top_positive_attributes": pack(pos_idx),
        "top_negative_attributes": pack(neg_idx),
    }
...
def build_per_k_feature_mapping(
        k: int,
        cntr_pca2: np.ndarray,
        feature_names: list[str],
        scaler: StandardScaler,
        pca2: PCA,
        top_cluster_features: int = 20
) -> dict:
    """
    For each cluster center in PCA-2D:
    1. inverse PCA back to standardized feature space
    2. inverse StandardScaler back to original feature scale
    3. rank features by standardized center magnitude
    """
    centers_std = pca2.inverse_transform(cntr_pca2)       # shape (k, n_features)
    centers_orig = scaler.inverse_transform(centers_std)  # shape (k, n_features)

    per_cluster = []
    for i in range(k):
        ranked = rank_cluster_features_from_center(
            center_std=centers_std[i],
            center_orig=centers_orig[i],
            feature_names=feature_names,
            top_n=top_cluster_features,
        )

        per_cluster.append({
            "cluster": int(i + 1),
            "center_pca2": {
                "PC1": float(cntr_pca2[i, 0]),
                "PC2": float(cntr_pca2[i, 1]),
            },
            "center_standardized_feature_space": {
                feature_names[j]: float(centers_std[i, j]) for j in range(len(feature_names))
            },
            "center_original_feature_scale": {
                feature_names[j]: float(centers_orig[i, j]) for j in range(len(feature_names))
            },
    ...
    return {
        "k": int(k),
        "method": "Fuzzy C-Means in PCA-2D with inverse projection to feature space",
        "ranking_explanation": (
            "Attributes are ranked using the inverse-projected cluster center in standardized "
            "feature space. Large positive values indicate attributes strongly associated with "
            "the cluster; large negative values indicate attributes under-expressed in the cluster; "
            "large absolute values indicate strongest differentiators overall."
        ),
...
def main():
    ...
    global_mapping = build_global_feature_mapping(
        csv_path=CSV_PATH,
    ...
    for k in ks_to_run:
        ...
        # Per-k feature mapping
        per_k_mapping = {
            "global_mapping_reference": os.path.join("..", "feature_mapping.json"),
            "features_used": feature_names,
            "feature_mapping_for_clusters": build_per_k_feature_mapping(
                k=k,
                cntr_pca2=cntr,
                feature_names=feature_names,
                scaler=scaler,
                pca2=pca2,
                top_cluster_features=TOP_CLUSTER_FEATURES,
            ),
        }
        with open(os.path.join(run_dir, f"feature_mapping_k_{k}.json"), "w", encoding="utf-8") as f:
            json.dump(per_k_mapping, f, indent=2)
        ...
        summary = {
            ...
            "feature_mapping_file": f"feature_mapping_k_{k}.json",
        }
    ...
    print(f"Global mapping:     {os.path.join(OUTPUT_DIR, 'feature_mapping.json')}")
